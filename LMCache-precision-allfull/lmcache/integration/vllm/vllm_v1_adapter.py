...
def extract_request_configs(sampling_params: SamplingParams) -> Optional[dict]:
    ...
    precision_plan = PrecisionLoadPlanner().plan(
        request_configs=request_configs,
        num_chunks=0,
    )
    return normalize_precision_request_configs(
        request_configs,
        precision_plan.default_tier,
    )
...
class LMCacheConnectorV1Impl:
        ...
        for idx, request in enumerate(metadata.requests):
            ...
            if self.use_layerwise:
                ...
                if self.enable_blending:
                    ...
                    next(layerwise_retriever)
                    if full_hit_layerwise_fallback:
                        logger.warning(
                            "Layerwise full-hit fallback to eager drain for request %s",
                            request.req_id,
                        )
                        ret_token_mask = None
                        while True:
                            try:
                                maybe_ret_token_mask = next(layerwise_retriever)
                            except StopIteration:
                                break
                            if maybe_ret_token_mask is not None:
                                ret_token_mask = maybe_ret_token_mask
            ...
            if (not self.use_layerwise) or full_hit_layerwise_fallback:
                # Check the result
    ...
    def get_num_new_matched_tokens(
        ...
        full_hit_tail_recompute = (
            os.getenv("LMCACHE_LAYERWISE_FULL_HIT_RECOMPUTE_TAIL_CHUNK", "False").lower()
            in ("1", "true", "yes", "on")
        )
        if (
            full_hit_tail_recompute
            ...
            and request.num_tokens > self._lmcache_chunk_size
        ):
            capped_hit_tokens = (
                (request.num_tokens - self._lmcache_chunk_size)
                // self._lmcache_chunk_size
                * self._lmcache_chunk_size
            )
            logger.warning(
                "Layerwise full-hit tail-recompute workaround enabled for request %s: "
                "cap LMCache hit tokens from %d to %d to keep vLLM on the prefill "
                "layer hook path",
                req_id,
                num_external_hit_tokens,
                capped_hit_tokens,
            )
            num_external_hit_tokens = capped_hit_tokens
