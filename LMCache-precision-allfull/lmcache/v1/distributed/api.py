...
from lmcache.v1.multiprocess.precision_codec import namespace_model_name_for_precision
...
def ipc_key_to_object_keys(
    ...
    precision_tiers: list[str] | tuple[str, ...] | None = None,
) -> list[ObjectKey]:
    """
    ...
    """
    if precision_tiers is not None and len(precision_tiers) != len(chunk_hashes):
        raise ValueError(
            "precision_tiers length must match chunk_hashes length: "
            f"{len(precision_tiers)} != {len(chunk_hashes)}"
        )

    storage_keys = []
    for chunk_idx, chunk_hash in enumerate(chunk_hashes):
        tier = precision_tiers[chunk_idx] if precision_tiers is not None else None
            ...
            for worker_id in range(ipc_key.world_size):
                ...
                storage_keys.append(
                    ObjectKey(
                        ...
                        model_name=namespace_model_name_for_precision(ipc_key.model_name, tier),
                        kv_rank=kv_rank,
                    )
                )
        else:
