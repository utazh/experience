...
_CONFIG_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "fidelity_auto_context_threshold": {
        "type": int,
        "default": 16384,
        "env_converter": int,
    },
    "fidelity_auto_kv_stall_threshold_ms": {
        "type": float,
        "default": 200.0,
        "env_converter": float,
    },
    ...
    "enable_fidelity_internal_state": {
        "type": bool,
        "default": False,
        "env_converter": _to_bool,
    },
    "fidelity_internal_state_promotion_min_access_count": {
        "type": int,
        "default": 2,
        "env_converter": int,
    },
    "fidelity_internal_state_enable_promotion": {
        "type": bool,
        "default": False,
        "env_converter": _to_bool,
    },
...
def _validate_config(self):
    ...
    if self.default_fidelity not in {"full", "base", "auto"}:
        raise ValueError(
            f"default_fidelity must be one of full/base/auto, got {self.default_fidelity}"
        )

    if self.base_codec not in {"fake", "int8"}:
        raise ValueError(
            f"base_codec must be one of fake/int8, got {self.base_codec}"
        )

    if self.fidelity_internal_state_promotion_min_access_count < 1:
        raise ValueError(
            "fidelity_internal_state_promotion_min_access_count must be >= 1, "
            f"got {self.fidelity_internal_state_promotion_min_access_count}"
        )
