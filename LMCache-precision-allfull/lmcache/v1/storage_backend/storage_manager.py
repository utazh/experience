...
def allocate_and_copy_objects(
    ...
    for key, src_memory_obj in zip(keys, src_memory_objs, strict=False):
        ...
        memory_obj = allocator_backend.allocate(
            src_memory_obj.get_shapes(),
            src_memory_obj.get_dtypes(),
        ...
        dst_raw_tensor = memory_obj.raw_tensor
        src_raw_tensor = src_memory_obj.raw_tensor
        if dst_raw_tensor is None or src_raw_tensor is None:
            logger.warning(
                "Allocated MemoryObj has None raw tensor, this is unexpected. "
        ...
        if stream is None:
            dst_raw_tensor.copy_(src_raw_tensor)
        else:
            with torch.cuda.stream(stream):
                dst_raw_tensor.copy_(src_raw_tensor, non_blocking=True)
