...
from lmcache.v1.multiprocess.precision_codec import (
    ...
    is_mp_int8_base_tier_enabled,
    precision_policy_from_env,
...
def _chunk_range_for_key(
    key: IPCCacheEngineKey,
    chunk_size: int,
) -> tuple[int, int]:
    if key.start % chunk_size != 0 or key.end % chunk_size != 0:
        raise ValueError(
            "MP precision routing requires chunk-aligned key range: "
            f"start={key.start}, end={key.end}, chunk_size={chunk_size}"
        )
    return key.start // chunk_size, key.end // chunk_size


def _total_chunks_for_key(
    key: IPCCacheEngineKey,
    chunk_size: int,
) -> int:
    return len(key.token_ids) // chunk_size


def _precision_tiers_for_key_range(
    key: IPCCacheEngineKey,
    chunk_size: int,
) -> tuple[str, ...]:
    total_chunks = _total_chunks_for_key(key, chunk_size)
    start_chunk, end_chunk = _chunk_range_for_key(key, chunk_size)
    return precision_tiers_for_range(total_chunks, start_chunk, end_chunk)


def _layout_desc_for_tier(
    layout_desc: MemoryLayoutDesc,
    tier: str,
) -> MemoryLayoutDesc:
    if is_mp_int8_base_tier_enabled(tier):
        return build_mp_base_layout_desc(layout_desc)
    return layout_desc


def _keys_per_chunk(key: IPCCacheEngineKey) -> int:
    return key.world_size if key.worker_id is None else 1

@dataclass
class _PrefetchSpanJob:
...
class MPCacheEngine:
    ...
    def store(
        ...
        tiers = _precision_tiers_for_key_range(key, self.chunk_size)
        obj_keys = ipc_key_to_object_keys(key, chunk_hashes, precision_tiers=tiers)
        ...
        with (
            ...
            for span_start, span_end, tier in group_precision_tier_spans(tiers):
                span_keys = obj_keys[span_start:span_end]
                if not span_keys:
                    continue
                span_layout = _layout_desc_for_tier(layout_desc, tier)
                reserved_dict.update(
                    self.storage_manager.reserve_write(span_keys, span_layout, "new")
                )

            base_encode_ms = 0.0
            full_chunks = 0
            base_chunks = 0
            baseline_full_bytes = 0
            actual_bytes = 0
            ...
            for idx, obj_key in enumerate(obj_keys):
                ...
                tier = tiers[idx]
                is_base_tier = is_mp_int8_base_tier_enabled(tier)
                ...
                with gpu_context.transfer_lock:
                    ...
                    chunk_full_bytes = tmp_buffer.numel() * tmp_buffer.element_size()
                    baseline_full_bytes += chunk_full_bytes
                    if is_base_tier:
                        encode_start = time.perf_counter()
                        encode_tensor_to_base_memory_obj(tmp_buffer, memory_obj)
                        gpu_context.stream.synchronize()
                        base_encode_ms += (time.perf_counter() - encode_start) * 1000
                        base_chunks += 1
                        actual_bytes += memory_obj.get_size()
                    else:
        ...
        if length := len(reserved_dict):
            ...
            if baseline_full_bytes > 0 and precision_policy_from_env() != "all-full":
                logger.info(
                    "MP_MIXED_STORE chunks=%d full_chunks=%d base_chunks=%d "
                    "full_bytes=%d actual_bytes=%d bytes_ratio=%.6f encode_ms=%.4f",
                    length,
                    full_chunks,
                    base_chunks,
                    baseline_full_bytes,
                    actual_bytes,
                    actual_bytes / baseline_full_bytes,
                    base_encode_ms,
                )
                if full_chunks == 0 and base_chunks > 0:
                    logger.info(
                        "MP_INT8_BASE_STORE chunks=%d full_bytes=%d base_bytes=%d "
                        "bytes_ratio=%.6f encode_ms=%.4f",
                        base_chunks,
                        baseline_full_bytes,
                        actual_bytes,
                        actual_bytes / baseline_full_bytes,
    ...
    def retrieve(
        ...
        def _retrieve_loop(
            ...
            for idx, (key, memory_obj) in enumerate(
                ...
                with gpu_context.transfer_lock:
                    chunk_full_bytes = (
                        tmp_gpu_buffer_.numel() * tmp_gpu_buffer_.element_size()
                    )
                    baseline_full_bytes[0] += chunk_full_bytes
                    if is_base_tier:
                        decode_start = time.perf_counter()
                        decode_base_memory_obj_to_tensor(memory_obj, tmp_gpu_buffer_)
                        base_decode_ms[0] += (time.perf_counter() - decode_start) * 1000
                        base_chunks[0] += 1
                        actual_bytes[0] += memory_obj.get_size()
                    else:
        ...
        with (
            ...
            try:
                with self.storage_manager.read_prefetched_results(
                    ...
                    _retrieve_loop(obj_keys, memory_objs, tiers)
        ...
        if baseline_full_bytes[0] > 0 and precision_policy_from_env() != "all-full":
            logger.info(
                "MP_MIXED_RETRIEVE chunks=%d full_chunks=%d base_chunks=%d "
                "full_bytes=%d actual_bytes=%d bytes_ratio=%.6f "
                "decode_enqueue_ms=%.4f",
                len(prefetched_keys),
                full_chunks[0],
                base_chunks[0],
                baseline_full_bytes[0],
                actual_bytes[0],
                actual_bytes[0] / baseline_full_bytes[0],
                base_decode_ms[0],
            )
            if full_chunks[0] == 0 and base_chunks[0] > 0:
                logger.info(
                    "MP_INT8_BASE_RETRIEVE chunks=%d full_bytes=%d base_bytes=%d "
                    "bytes_ratio=%.6f decode_enqueue_ms=%.4f",
                    base_chunks[0],
                    baseline_full_bytes[0],
                    actual_bytes[0],
                    actual_bytes[0] / baseline_full_bytes[0],
    ...
    def lookup(
        ...
        Hashes the key, submits one prefetch task per precision span to the
        storage manager, and returns a job ID that can be polled via
        ...
        if layout_desc is None:
            ...
            return self._register_prefetch_job(
                _PrefetchJob(spans=(), world_size=1, request_id=key.request_id)
            )
        ...
        tiers = precision_tiers_for_range(total_chunks, 0, total_chunks)
        obj_keys = ipc_key_to_object_keys(key, chunk_hashes, precision_tiers=tiers)
        keys_per_chunk = _keys_per_chunk(key)

        span_jobs: list[_PrefetchSpanJob] = []
        for span_start, span_end, tier in group_precision_tier_spans(tiers):
            key_start = span_start * keys_per_chunk
            key_end = span_end * keys_per_chunk
            span_keys = obj_keys[key_start:key_end]
            if not span_keys:
                continue
            handle = self.storage_manager.submit_prefetch_task(
                span_keys,
                _layout_desc_for_tier(layout_desc, tier),
                extra_count=extra_count,
            )
            span_jobs.append(
                _PrefetchSpanJob(
                    handle=handle,
                    chunk_count=span_end - span_start,
                    keys=span_keys,
                )
            )
    ...
    def query_prefetch_status(
        ...
        Returns the contiguous prefix chunk count when the per-span prefetches
        are complete, or None if any span is still in progress. Mixed routing
        computes prefix hits in original chunk order so a later span hit cannot
        hide a miss in an earlier span.
        ...
        for span in job.spans:
            found_count = self.storage_manager.query_prefetch_status(span.handle)
            if found_count is None:
                return None
            span_hits.append(min(found_count // job.world_size, span.chunk_count))

        found_count = 0
        for span, hit_count in zip(job.spans, span_hits, strict=True):
            found_count += hit_count
            if hit_count < span.chunk_count:
                break
