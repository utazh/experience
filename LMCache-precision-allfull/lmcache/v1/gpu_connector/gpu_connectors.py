...
class VLLMPagedMemGPUConnectorV2(GPUConnectorInterface):
    ...
    def _initialize_pointers(self, kv_caches: List[torch.Tensor]) -> torch.Tensor | List[torch.Tensor]:
        ...
        if getattr(lmc_ops, "PYTHON_FALLBACK", False):
            return kv_caches
...
class VLLMPagedMemGPUConnectorV3(GPUConnectorInterface):
    ...
    def _initialize_kv_cache_pointers(self):
        ...
        for group in self.metadata.kv_layer_groups_manager.kv_layer_groups:
            group_kv_caches = [
                t for i, t in enumerate(self.kvcaches) if i in group.layer_indices
            ]
            if getattr(lmc_ops, "PYTHON_FALLBACK", False):
                self.group_kv_cache_pointers_on_gpu.append(group_kv_caches)
            else:
                num_layers = group.num_layers
                kv_cache_pointers = torch.empty(num_layers, dtype=torch.int64, device="cpu")
                kv_cache_pointers.numpy()[:] = [t.data_ptr() for t in group_kv_caches]
